...
void f3d_gyro_interface_init() {
	/**********************************************************************/
	/************** CODE HERE *********************************************/
	GPIO_InitTypeDef SPIPins_InitStructure;
	GPIO_StructInit(&SPIPins_InitStructure);
	SPIPins_InitStructure.GPIO_Pin = GPIO_Pin_5 | GPIO_Pin_6 | GPIO_Pin_7;
	SPIPins_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	SPIPins_InitStructure.GPIO_Mode = GPIO_Mode_AF;
	SPIPins_InitStructure.GPIO_OType = GPIO_OType_PP;
	SPIPins_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOA, ENABLE);
	GPIO_Init(GPIOA,&SPIPins_InitStructure);
	...
	//CS PE3
	GPIO_InitTypeDef CS_InitStructure;
	GPIO_StructInit(&CS_InitStructure);
	CS_InitStructure.GPIO_Pin = GPIO_Pin_3;
	CS_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	CS_InitStructure.GPIO_OType = GPIO_OType_PP;
	CS_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	CS_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOE, ENABLE);
	GPIO_Init(GPIOE, &CS_InitStructure);
	
	//set the CS high
	GPIOE->BSRR = GPIO_Pin_3;
	//GPIO_SetBits(GPIOE, GPIO_Pin_3); // this should be a macro for the above line
	/**********************************************************************/
...
void f3d_gyro_init(void) {
	...
	ctrl1 |= (uint8_t) (((uint8_t)0x00)|((uint8_t)0x30)|((uint8_t)0x08)|((uint8_t)0x07));
	// CTRL4 Register
	// Bit 7 Block Update: Continuous */
	// Bit 6 Endianess: LSB first */
	// Bit 5:4 Full Scale: 500 dps */
	ctrl4 |= (uint8_t) (((uint8_t)0x00)|((uint8_t)0x00)|((uint8_t)0x10));
